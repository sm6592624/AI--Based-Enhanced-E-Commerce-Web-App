...
const AiStylistPage = () => {
    ...
    // Debug and validate config on component mount
    React.useEffect(() => {
        debugConfig();
        try {
            validateConfig();
    ...
    const getStyleRecommendations = async () => {
        ...
        try {
                            ...
                            content: [
                                {
                                    ...
                                    Please provide specific clothing recommendations with explanations for why they would work well.`
                                },
                                {
                                    type: "image_url",
                                    image_url: {
            ...
            console.log('OpenAI API data received:', data);
            ...
            setError(`Failed to get AI recommendations: ${err.message}. Using demo recommendations instead.`);
            setRecommendations(mockAIRecommendations);
            setUsingMockData(true);
        } finally {
            setLoading(false);
    ...
    return (
        <div className="container mx-auto px-6 py-12">
            <div className="text-center mb-12">
                ...
                {(config.useMockData || !config.openaiApiKey) && (
                    <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mt-4 max-w-md mx-auto">
                        Demo mode: AI recommendations will be simulated
                    </div>
                )}
            </div>
                    ...
                    <div className="prose max-w-none">
                        {recommendations.map((rec, index) => (
                            <div key={index} className="mb-4 p-4 bg-gray-50 rounded-lg">
                                <p className="whitespace-pre-wrap text-gray-700">{rec}</p>
                            </div>
                        ))}
                    </div>
                </div>
